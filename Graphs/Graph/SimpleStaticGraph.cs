...
namespace GraphsRender.Graph
{
    ...
    public class SimpleStaticGraph<TVertexDataType, TEdgeDataType> : ICloneable
    {
        ...
        private abstract class GeneralGraph
        {
            ...
            public abstract TEdgeDataType GetEdgeData(int v1, int v2);
            ...
            /// <summary>
            /// Получение коэффициента насыщения графа
            /// </summary>
            /// <returns></returns>
            ...
            {
                var maxEdges = VertexCount*(VertexCount - 1);
                if (Direction == GraphType.NotOriented)
                {
                    maxEdges /= 2;
                }

                if(randomEdgesCount > maxEdges)
                {
                    randomEdgesCount = maxEdges;
                }
                ...
                for (int i = 0; i != randomEdgesCount; i++)
                {
                    ...
                    if(!InsertEdge(v1, v2)) //Если неудачная вставка - повторяем попытку
                    {
                        i--;
                    }
                }
            ...
            public abstract class GeneralIterator
            {
                ...
                public abstract Edge GetCurrentEdge();
            }
        }
        ...
        private class MatrixGraph : GeneralGraph
        {
            ...
            {
                if (v1 == v2) return false; //Запрещаем создание петель
            ...
            public override TEdgeDataType GetEdgeData(int v1, int v2)
            {
                if (matrix[v1, v2] != null)
                {
            ...
            public class Iterator : GeneralIterator 
            {
                ...
                public override Edge GetCurrentEdge()
                {
                    if (IsAlive())
                    {
                        return Graph.matrix[V, I];
                    }

                    throw new NullReferenceException("Итератор вышел за пределы данных!");
                }
            }
        }
        ...
        private class ListGraph : GeneralGraph
        {
            ...
            public class Iterator : GeneralIterator
            {
                ...
                public override Edge GetCurrentEdge()
                {
                    if(IsAlive())
                    {
                        return _listIter.Current.Value;
                    }

                    throw new NullReferenceException("Итератор за пределами данных!");
                }
            }
        }
        ...
        public class Iterator
        {
            ...
            /// <summary>
            /// Получение текущего ребра
            /// </summary>
            /// <returns></returns>
            public Edge GetCurrentEdge()
            {
                return _iter.GetCurrentEdge();
            }
        }
        #endregion
        ...
        /// <param name="v2">Конечная вершина</param>
        /// <returns></returns>
        public TEdgeDataType GetEdgeData(int v1, int v2)
        {
            return _graph.GetEdgeData(v1, v2);
        }
            ...
            if (Dense() == GraphFormat.MatrixGraph)
            {
                ...
                for (int i = 0; i != _graph.VertexCount; i++)
                {
                    ...
                    while (iter.Current() != -1)
                    {
                        ...
                        g.SetEdge(i, iter.Current(), _graph.GetEdgeData(i, iter.Current()));
                        iter.Next();
                    }
                }
        ...
        public List<Edge> GetEdges()
        {
            ...
            {
                var iter = new Iterator(this, v);
                iter.Begin();

                for(int k = iter.Current(); k != -1; iter.Next(), k = iter.Current())
                {
                    if(iter.GetCurrentEdge().StartVertex == v)
                    {
                        edgesList.Add(iter.GetCurrentEdge());
                    }
                }
            }
